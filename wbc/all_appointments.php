                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <strong class="card-title">All Sessions</strong>
                        </div>
                        <div class="card-body">
                    ...
                    <tbody>
                    	<?php
					        ...
					        while ($row = mysql_fetch_assoc($result)){
                                array_push($aids, $row['AID']);
					        	echo "<tr>";
                                echo "<td>WBC-A$row[AID]</td>";
                                echo "<td>WBC-S$row[SID]</td>";
                                echo "<td>" . stripcslashes($row["concern"]) . "</td>";
                                echo "<td><div class='icon-container' style='width:auto;' ><a data-toggle='modal' data-target='#WBCS$row[AID]' href=''><span class='ti-google'></span><span class='icon-name'> Show Resolution</span></a></div></td>";
                                echo "</tr>";
                  ...
                  <?php
                        ...
                        $query = "SELECT * FROM application WHERE $content";
                        $result = mysql_query($query, $db_connectionn);

                        while ($row = mysql_fetch_assoc($result)){
                            $aprob_area = stripcslashes($row["aprob_area"]);
                            $aprob_area_remark = stripcslashes($row["aprob_area_remark"]);
                            $amedical_state_remark = stripcslashes($row["amedical_state_remark"]);
                            $ahistory = stripcslashes($row["ahistory"]);
                            $ainterventions = stripcslashes($row["ainterventions"]);

                            $output=<<<EOT
                <div class="modal fade" id="WBCS$row[AID]" tabindex="-1" role="dialog" aria-labelledby="scrollmodalLabelWBCS$row[AID]" aria-hidden="true">
                    <div class="modal-dialog modal-lg" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="scrollmodalLabelWBCS$row[AID]">Session WBC-A$row[AID]'s Resolution</h5>
                            ...
                            <div class="modal-body">
                                <p>
                                    <hr/>
                                    <b>Problematic Area:</b> $aprob_area<br/>
                                    <b>Problematic Area Remarks:</b> $aprob_area_remark
                                    <hr/>

                                    <b>Mental State Examination:</b> (Appearance, attitude, behaviour, speech, mood, thoughtprocess, perception, etc)<br/>
                                    <b>Mental State Examination Remarks:</b> $amedical_state_remark
                                    <hr/>

                                    <b>History of Problematic Area:</b> $ahistory<br/>
                                    <b>Interventions:</b> $ainterventions<br/>
                                </p>
                            </div>
